...
class AbstractField {
  ...
  buildFieldRule(position, field, path, source, rules) {
    ...
    const baseTypes = ['boolean', 'integer', 'number', 'string', 'enum', 'id', 'email', 'password', 'url', 'datetime', 'date'];
    ...
    if (field.type.startsWith('array') && baseTypes.includes(itemType)) {
      ...
      const transfer = ['convertType', 'default', 'allowEmpty', 'format', 'trim', 'compare'].reduce((acc, key) => { 
        acc[key] = itemObj[key]; 
        delete itemObj[key]; 
        return acc; 
      }, {});
      itemObj.rule = Object.assign({ type: itemType }, transfer);
    }
  ...
  priorityEnumType(values) {
    if (values.filter(f => f === 'string').length > 0) return 'string';
    if (values.filter(f => f === 'number').length > 0) return 'number';
    if (values.filter(f => f === 'integer').length > 0) return 'integer';
    return 'string';
  }
  ...
  buildFieldSwagger(position, field, path, source) {
    ...
    itemObj.required = position === 'path' ? true : itemObj.required;
    field.type = field.type ?? '';
    field.type = ['email', 'password', 'url', 'datetime', 'date'].includes(field.type) ? 'string' : field.type;
    field.type = field.type === 'id' ? 'integer' : field.type;
    field.type = field.type === 'enum' ? this.priorityEnumType(Array.from(new Set((field.enum || []).map(m => this.transformEnumType(m))))) : field.type;
    itemObj.type = field.type;
