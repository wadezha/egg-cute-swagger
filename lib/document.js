...
class DocumentClient extends AbstractField {
  ...
  buildParameterSwagger(router, definitions) {
    ...
    if (router.request.length === 1  && router.request.filter(f => f.position === 'query' && Object.keys(definitions).includes(f.type)).length > 0) {
      let rules = definitions[router.request[0].type].properties;
      rules = Object.keys(rules).map(m => Object.assign({ position: 'query', name: m, desc: rules[m]['description'] }, rules[m]));
      router.request = rules.map(m => Object.assign({}, ...Object.keys(m).filter(f => f !== 'description').map(f => ({ [f]: m[f] }))));
    }
