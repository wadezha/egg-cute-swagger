...
class DocumentClient extends AbstractField {
  ...
  buildTagPath(config, definitions) {
    const routers = appRouters.filter(f => !f.ignored);
    const tags = [];
    const paths = {};
    for (const router of routers) {
      ...
      const permission = `<p><b>Permission</b> <span style="font-size:13px;padding:0 6px 3px 6px;border-radius:4px;background-color:#f2eefd;color:#9373ee;">${router.handlerPerm.map(m => m.code).join(', ')}</span></p>`;
      const swagger = {};
      swagger.tags = [tag];
      swagger.summary = router.handlerSummary || router.handler;
      swagger.description = `<p style="font-size:13px;">${router.handlerDesc}</p> ${router.handlerPerm.length > 0 ? permission : ''}`;
  ...
  buildParameterSwagger(router, definitions) {
    ...
    let queries = router.request.filter(f => f.position === 'query');
    // query只有一条 且 是对象类型, 需要将对象拆分成数组
    if (queries.length === 1  && queries.filter(f => f.position === 'query' && Object.keys(definitions).includes(f.type)).length > 0) {
      let rules = definitions[queries[0].type].properties;
      rules = Object.keys(rules).map(m => Object.assign({ position: 'query', name: m, desc: rules[m]['description'] }, rules[m]));
      queries = rules.map(m => Object.assign({}, ...Object.keys(m).filter(f => f !== 'description').map(f => ({ [f]: m[f] }))));
    }
    router.request = [...queries, ...router.request.filter(f => f.position !== 'query')];
  ...
  checkTypeField(position, field, path, source, baseTypes) {
    ...
    // query: 只支持 integer/string/boolean/number/User, array[string]
    // path/headers: 只支持 integer/string/boolean/number, array[string]
    if (['path', 'headers'].includes(position) && !field.type.startsWith('array') && !baseTypes.includes(field.type)) {
      throw new Error(`[egg-cute-swagger] ${path}.type does not support`);
    }
    if (['query', 'path', 'headers'].includes(position) && field.type.startsWith('array') && !baseTypes.includes(itemType)) {
      throw new Error(`[egg-cute-swagger] ${path}.itemType does not support`);
    }
  }
  ...
  mergeParameterRule(position, router, params, source) {
    ...
    if (position === 'body' && params.length === 1 && params.find(f => f.type === 'object' || f.type === 'array')) {
      // 250726 有名称时保留名称, 否则 { k: ['xx', 'xx3'] } 规则会被转成 { body: ['xx', 'xx3'] } 导致校验不能通过
      params = params.map(m => Object.assign(m, m.name ? {} : { name: position }));
    }

    // query只有一条 且 是对象类型, 需要将对象拆分成数组
    if (position === 'query' && params.length === 1 && params.find(f => f.type === 'object')) {
      params = Object.keys(params[0].rule).map(m => Object.assign({ position: 'query', name: m, }, params[0].rule[m]));
    }
  ...
  buildParameterRule(router, source, routes) {
    ...
    return Object.assign({}, ...positions.map(m => ({ [m]: this.mergeParameterRule(m, router, requests.filter(f => f.in === m), source) })));
  }
