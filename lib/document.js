...
class DocumentClient extends AbstractField {
  ...
  buildTagPath(config, definitions) {
    ...
    const groups = [...new Set(routers.map(m => m.controllerGroup))];
    const tags = {};
    const paths = {};
    for (const router of routers) {
      const group = `${router.controllerGroup || '-'}`;
      tags[group] = tags.hasOwnProperty(group) ? tags[group] : [];

      if (!tags[group].find(f => f.name === (router.controllerSummary || router.controller))) {
        tags[group].push({ name: router.controllerSummary || router.controller, description: router.controllerDesc });
      }

      const swagger = {};
      swagger.tags = [router.controllerSummary || router.controller];
      swagger.summary = router.handlerSummary || router.handler;
      swagger.description = `${router.permission} ${router.handlerDesc}`;
      ...
      util.ensurePathExists(paths, `${group},${router.url},${router.method}`, {});
      paths[group][router.url][router.method] = swagger;
    }
    return { groups, tags, paths };
  }
  ...
  buildDocumentRule() {
    ...
    for (const router of appRouters) {
      ...
      paths[`${router.url}.${router.method}`] = rules;
    }
